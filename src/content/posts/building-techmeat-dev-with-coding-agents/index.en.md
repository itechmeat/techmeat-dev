...
I deliberately kept design as a separate phase so I wouldn't be mixing it with architecture and content in a single task. Colleagues recommended the [Impeccable](https://impeccable.style/) skill system — it helps the agent build a more thoughtful visual instead of the default AI aesthetic.
...
Next — posters, so links to the blog look good when shared on social. I plugged in [Pencil](https://www.pencil.dev/) (their MCP is excellent) and asked Claude Code to build a [poster system](https://github.com/itechmeat/techmeat-dev/blob/master/design/posters.pen) and export it into the project. Some manual touch-ups were needed, but overall — fast and clean.
