...
## От ручных workflow к Dark Factory
...
Мне хотелось прийти к более жёсткой модели, которая всё чаще звучит как Dark Factory: на вход подаётся идея фичи, на выходе - фича реализована, протестирована и задеплоена. Не «агент написал кусок кода», а фабрика, которая умеет сама разложить работу на стадии и провести её через процесс.

До полноценной Dark Factory ещё далеко. Но первый практический шаг уже есть: Hermes, поднятый на VPS, с агентами под разные задачи, скиллами, Telegram-интерфейсом и дешёвой маршрутизацией моделей через OmniRoute.
...
Пока это ещё не Dark Factory. Но это уже не просто «чат с моделью».
...
- превратить текущие workflow в более автономную Dark Factory;
