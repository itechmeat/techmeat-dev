...
И главное - это не должно зависеть только от «памяти модели». Мне нужна простая, проверяемая, файловая система знаний, которую можно открыть руками, прочитать в Obsidian, синхронизировать, закоммитить частично или вообще не коммитить.
...
Дальше проект начал меняться под давление интеграций.

## От Hermes к универсальному плагину

Hermes остался основным runtime. Именно под него проект и задумывался: установить плагин, указать vault, дать агенту инструменты и заставить его писать важные события в Second Brain.
...
Например, OpenClaw сначала получил native plugin compatibility, но рантайм оказался строже, чем ожидалось. Пришлось добавлять `name` внутрь tool objects, делать `register()` синхронным, а потом переписывать OpenClaw-плагин на чистый JavaScript без `child_process`, потому что security scanner блокировал subprocess.

Следующая большая тема - identity. Если в дневнике написано просто `@agent`, такой лог почти бесполезен. Поэтому в `0.6.0` появился workflow с агентскими именами: `o2b init --agent-name`, регистрация в `AI Wiki/identity/agents.md` и проверка, что записи в Daily получают нормальный `@agent-name`, а не placeholder.
...
В `0.7.0` агент убрал дублирование: Hermes, Claude Code, Codex и OpenClaw теперь потребляют общие модули из `src/core/`. CLI живёт в `src/cli/`, MCP - в `src/mcp/`, OpenClaw entry собирается из TypeScript в JS bundle через `bun build`.
...
Это как раз тот момент, где видно преимущество агентского workflow. Человеку неприятно руками перетаскивать один и тот же код между рантаймами и переписывать тесты. Агенту - нормально, если дать чёткую цель, ограничения и проверку результата.
...
Самое ценное даже не список команд. Ценно то, что у агентов появился общий протокол памяти: когда случилось что-то долговечное - код, фикс, конфиг, контент, исследовательский вывод, дизайн-решение - это нужно записать так, чтобы future-me и future-agent могли найти это позже.
...
Извините, но этот пост был написан тоже тем же агентом Hermes. Руками написан только этот абзац и мой [пост на Facebook](https://www.facebook.com/reel/1355271143340726/). Я просто попросил агента склонировать к себе мой блог как обычный проект, посмотреть историю коммитов и взять Facebook-пост как базу. И конечно же я перечитал и поправил текст перед публикацией. И не говорите что это без души, я вкладываю душу в агента.
